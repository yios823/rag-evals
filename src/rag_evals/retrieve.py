...
class HybridRetriever:
    def __init__(self, index_dir: Path) -> None:
        ...
        self._chunks = [json.loads(line) for line in (index_dir / "chunks.jsonl").read_text().splitlines()]
    ...
    def search(self, query: str, top_n: int | None = None) -> list[Hit]:
        ...
        with trace("retrieve.hybrid", query=query, top_n=n) as span:
            ...
            fused = self._reciprocal_rank_fusion([dense, bm25], k=CONFIG.rrf_k)
            hits = [
                Hit(
            ...
            span["dense_top1"] = self._chunks[dense[0][0]]["chunk_id"] if dense else None
            span["bm25_top1"] = self._chunks[bm25[0][0]]["chunk_id"] if bm25 else None
    ...
    def _reciprocal_rank_fusion(
        ...
        """Standard RRF: score(i) = sum over rankings of 1 / (k + rank_in_ranking(i))."""
